...
class ArtifactService:
    ...
    def link_artifact_to_story(
        self, story_id: str, uri: str, relation: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            uri = URIValidator.validate_uri_or_raise(
                uri, max_length=self.MAX_URI_LENGTH
            )
    ...
    def get_artifacts_for_story(self, story_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            artifacts = self.artifact_repository.find_artifacts_by_story_id(
                story_id.strip()
            )
