...
class DependencyRepository:
    ...
    def add_dependency(self, story_id: str, depends_on_story_id: str) -> bool:
        ...
        try:
            ...
            existing = self.db_session.execute(
                text("SELECT 1 FROM story_dependencies WHERE story_id = :story_id AND depends_on_story_id = :depends_on_story_id"),
            ...
            self.db_session.execute(
                text("INSERT INTO story_dependencies (story_id, depends_on_story_id) VALUES (:story_id, :depends_on_story_id)"),
                {"story_id": story_id, "depends_on_story_id": depends_on_story_id}
            )
    ...
    def story_exists(self, story_id: str) -> bool:
        ...
        """
        try:
            return self.db_session.query(Story).filter(Story.id == story_id).first() is not None
        except SQLAlchemyError as e:
            raise e
    ...
    def remove_dependency(self, story_id: str, depends_on_story_id: str) -> bool:
        ...
        try:
            result = self.db_session.execute(
                text("DELETE FROM story_dependencies WHERE story_id = :story_id AND depends_on_story_id = :depends_on_story_id"),
                {"story_id": story_id, "depends_on_story_id": depends_on_story_id}
            )
