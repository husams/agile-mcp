...
class ArtifactRepository:
    ...
    def find_artifact_by_id(self, artifact_id: str) -> Optional[Artifact]:
        ...
        """
        try:
            return self.db_session.query(Artifact).filter(Artifact.id == artifact_id).first()
        except SQLAlchemyError as e:
            raise e
    ...
    def find_artifacts_by_story_id(self, story_id: str) -> List[Artifact]:
        ...
        """
        try:
            return self.db_session.query(Artifact).filter(Artifact.story_id == story_id).all()
        except SQLAlchemyError as e:
            raise e
    ...
    def delete_artifact(self, artifact_id: str) -> bool:
        ...
        """
        try:
            artifact = self.db_session.query(Artifact).filter(Artifact.id == artifact_id).first()
            if artifact:
