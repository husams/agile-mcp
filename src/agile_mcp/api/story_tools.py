...
from ..models.response import StoryResponse, DoDChecklistResponse
...
def register_story_tools(mcp: FastMCP) -> None:
    ...
    try:
        ...
        logger.error("Failed to create database tables", error=str(e), operation="register_story_tools")
        raise
    ...
    def create_story(epic_id: str, title: str, description: str, acceptance_criteria: List[str]) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                ...
                **create_request_context(request_id=request_id, tool_name="backlog.createStory"),
    ...
    def get_story(story_id: str) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                ...
                **create_request_context(request_id=request_id, tool_name="backlog.getStory"),
    ...
    def update_story_status(story_id: str, status: str) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                ...
                **create_request_context(request_id=request_id, tool_name="backlog.updateStoryStatus"),
                **create_entity_context(story_id=story_id),
                new_status=status
            )
    ...
    def execute_story_dod_checklist(story_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                checklist_items = [
                    {
                        ...
                        "status": "PASSED" if story_dict["status"] in ["Review", "Done"] else "FAILED",
                        "required": True
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "category": "Quality",
                        "status": "PASSED" if story_dict["status"] == "Done" else "FAILED",
                        "required": True
                    },
                    {
                        "id": "no_regressions",
                    ...
                    },
                    {
                        ...
                        "status": "PASSED" if story_dict["status"] == "Done" else "PARTIAL",
                        "required": False
                    },
                    {
                        "id": "api_docs_updated",
                ...
                total_passed = [item for item in checklist_items if item["status"] == "PASSED"]
                total_failed = [item for item in checklist_items if item["status"] == "FAILED"]
                ...
                for item in checklist_items:
                    if item["status"] == "FAILED" and item["required"]:
                        recommendations.append(f"Complete: {item['name']} - {item['description']}")
                    elif item["status"] == "PARTIAL":
                        recommendations.append(f"Review: {item['name']} - {item['description']}")
                
                if overall_status == "PASSED":
                    recommendations = ["Story meets all Definition of Done criteria and is ready for completion."]
                ...
                summary = {
                    ...
                    "partial_items": len([item for item in checklist_items if item["status"] == "PARTIAL"]),
