...
class Story(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("length(description) <= 2000", name="ck_story_description_length"),
        CheckConstraint("status IN ('ToDo', 'InProgress', 'Review', 'Done')", 
    ...
    def validate_status(self, key, status):
        ...
        if status not in valid_statuses:
            raise ValueError(f"Story status must be one of: {', '.join(valid_statuses)}")
        return status
