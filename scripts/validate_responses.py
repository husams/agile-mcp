...
def validates_json_return(node: ast.FunctionDef) -> bool:
    ...
    for ret in returns:
        ...
        if isinstance(ret.value, ast.Call):
            if (isinstance(ret.value.func, ast.Attribute) and 
                ret.value.func.attr == "dumps" and
                isinstance(ret.value.func.value, ast.Name) and
                ret.value.func.value.id == "json"):
                continue
            
            # Check for mcp_response helper usage
            if (isinstance(ret.value.func, ast.Name) and 
                ret.value.func.id in ["mcp_response", "success_response", "error_response"]):
                continue
...
def check_pydantic_model_usage(node: ast.FunctionDef) -> List[str]:
    ...
    for child in ast.walk(node):
        if (isinstance(child, ast.Call) and
            ...
            child.func.attr == "model_dump_json"):
    ...
    for child in ast.walk(node):
        if isinstance(child, (ast.Dict, ast.List)) and len(getattr(child, 'keys', [])) > 3:
            has_complex_data = True
            break
    
    if has_complex_data and not has_pydantic_serialization:
        errors.append(f"Function '{node.name}' has complex data structures but no Pydantic serialization")
