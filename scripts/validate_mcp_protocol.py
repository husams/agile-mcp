...
def validate_tool_name(name: str) -> List[str]:
    ...
    if not re.match(r'^[a-zA-Z][a-zA-Z0-9_-]*$', name):
        errors.append(f"Tool name '{name}' must start with letter and contain only alphanumeric, underscore, or hyphen characters")
    ...
    if '--' in name or '__' in name:
        errors.append(f"Tool name '{name}' should not contain consecutive hyphens or underscores")
...
def validate_tool_description(description: str) -> List[str]:
    ...
    if len(description) < 10:
        errors.append(f"Tool description too short: '{description}' (minimum 10 characters)")
    
    if len(description) > 500:
        errors.append(f"Tool description too long: {len(description)} characters (maximum 500)")
    
    if not description[0].isupper():
        errors.append(f"Tool description should start with capital letter: '{description}'")
...
def validate_docstring(node: ast.FunctionDef) -> List[str]:
    ...
    for param in params:
        if param not in docstring and param != "self":
            errors.append(f"Function '{node.name}' docstring missing parameter '{param}' documentation")
