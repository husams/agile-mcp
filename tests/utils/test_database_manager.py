...
class DatabaseManager:
    ...
    def create_memory_database(self, test_id: Optional[str] = None) -> Tuple[Engine, sessionmaker]:
        """
    ...
    def create_shared_memory_database(self, test_suite_id: str = "integration") -> Tuple[Engine, sessionmaker]:
        """
        ...
        if cache_key not in self._engines:
            ...
            db_url = f"sqlite:///:memory:?cache=shared&uri=true&suite_id={test_suite_id}"
    ...
    def create_file_database(self, test_id: Optional[str] = None) -> Tuple[Engine, sessionmaker, str]:
        """
        ...
        temp_file = tempfile.NamedTemporaryFile(
            delete=False, 
            suffix=f'_{test_id}.db',
            prefix='test_agile_mcp_'
        )
        ...
        try:
            ...
            with engine.connect() as conn:
                for table_name in metadata.tables.keys():
                    try:
                        conn.execute(text(f"SELECT COUNT(*) FROM {table_name}")).fetchone()
                    except Exception:
    ...
    def measure_performance(self, test_type: str, test_id: Optional[str] = None) -> Dict[str, float]:
        """
