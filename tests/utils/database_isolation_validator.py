...
class DatabaseIsolationValidator:
    ...
    def validate_clean_database_state(self, engine: Engine) -> Dict[str, Any]:
        ...
        try:
            with engine.connect() as conn:
                ...
                tables_to_check = ["epics", "stories", "artifacts", "story_dependencies"]
                ...
                for table_name in tables_to_check:
                    try:
                        ...
                        if result > 0:
                            ...
                            validation_result["issues"].append(f"Table {table_name} contains {result} records")
                            
                            # Get sample contamination data
                            sample_result = conn.execute(text(f"SELECT * FROM {table_name} LIMIT 3")).fetchall()
                        ...
                        validation_result["issues"].append(f"Error checking table {table_name}: {str(e)}")
    ...
    def validate_production_isolation(self, test_engine: Engine) -> Dict[str, Any]:
        ...
        try:
            ...
            if production_db_path in test_url and not validation_result["test_db_info"]["is_memory"]:
                validation_result["is_isolated"] = False
                validation_result["issues"].append("Test database is using production database file")
            
            # Validate test environment markers
            test_mode = os.getenv("MCP_TEST_MODE")
            if not test_mode:
                validation_result["issues"].append("MCP_TEST_MODE environment variable not set")
            
            # Check for test database URL override
            test_db_url = os.getenv("TEST_DATABASE_URL")
            if test_db_url and production_db_path in test_db_url:
                validation_result["is_isolated"] = False
                validation_result["issues"].append("TEST_DATABASE_URL points to production database")
    ...
    def validate_test_isolation_between_sessions(self, test_type: str = "unit") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if test_type == "unit":
                engine1, factory1 = self.manager.create_memory_database("isolation_test_1")
                engine2, factory2 = self.manager.create_memory_database("isolation_test_2")
            elif test_type == "integration":
                engine1, factory1 = self.manager.create_shared_memory_database("isolation_suite_1")
                engine2, factory2 = self.manager.create_shared_memory_database("isolation_suite_2")
            elif test_type == "e2e":
                engine1, factory1, path1 = self.manager.create_file_database("isolation_test_1")
                engine2, factory2, path2 = self.manager.create_file_database("isolation_test_2")
            else:
            ...
            try:
                ...
                found_in_session1 = session1.query(Epic).filter_by(id=f"isolation-test-epic-{test_id}").first()
                validation_result["session_results"].append({
                    "session": 1,
                    "found_own_data": found_in_session1 is not None
                })
                
                # Verify data does NOT exist in second session (for unit/e2e tests)
                found_in_session2 = session2.query(Epic).filter_by(id=f"isolation-test-epic-{test_id}").first()
                session2_contaminated = found_in_session2 is not None
                
                validation_result["session_results"].append({
                    "session": 2,
                    "found_other_session_data": session2_contaminated
                })
                
                if session2_contaminated and test_type in ["unit", "e2e"]:
                    validation_result["is_isolated"] = False
                    validation_result["issues"].append("Cross-session contamination detected")
                ...
                if test_type == "integration":
                    validation_result["integration_note"] = "Integration tests may share data depending on configuration"
            ...
            validation_result["is_isolated"] = False
            validation_result["issues"].append(f"Session isolation validation error: {str(e)}")
        
        return validation_result
    
    def validate_thread_safety(self, test_type: str = "unit", thread_count: int = 5) -> Dict[str, Any]:
        """
        ...
        def thread_test_function(thread_id: int):
            ...
            try:
                ...
                if test_type == "unit":
                    engine, factory = self.manager.create_memory_database(f"thread_test_{thread_id}")
                elif test_type == "integration":
                    engine, factory = self.manager.create_shared_memory_database(f"thread_suite_{thread_id}")
                elif test_type == "e2e":
                    engine, factory, db_path = self.manager.create_file_database(f"thread_test_{thread_id}")
                else:
                    raise ValueError(f"Unknown test type: {test_type}")
                ...
                try:
                    ...
                    # Verify data exists
                    found = session.query(Epic).filter_by(id=f"thread-epic-{thread_id}").first()
                    if found is None:
        ...
        if failed_threads > 0:
            ...
            validation_result["issues"].append(f"{failed_threads} out of {thread_count} threads failed")
        ...
        validation_result["performance_metrics"] = {
            ...
            "average_thread_time": sum(execution_times) / len(execution_times) if execution_times else 0,
    ...
    def generate_isolation_report(self, test_types: List[str] = None) -> Dict[str, Any]:
        ...
        for test_type in test_types:
            type_report = {
                ...
                "session_isolation": self.validate_test_isolation_between_sessions(test_type),
                "thread_safety": self.validate_thread_safety(test_type, thread_count=3)
            }
            
            # Check production isolation for file-based tests
            if test_type == "e2e":
                engine, _, db_path = self.manager.create_file_database(f"prod_isolation_test")
                type_report["production_isolation"] = self.validate_production_isolation(engine)
                try:
                    os.unlink(db_path)
                except OSError:
                    pass
            ...
            for validation_name, validation_result in type_report.items():
                if isinstance(validation_result, dict):
                    passed = validation_result.get("is_clean", True) and \
                            validation_result.get("is_isolated", True) and \
                            validation_result.get("is_thread_safe", True)
                    if not passed:
                        all_passed = False
                        issues = validation_result.get("issues", [])
                        report["summary"]["issues"].extend(
                            [f"{test_type}.{validation_name}: {issue}" for issue in issues]
                        )
