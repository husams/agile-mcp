...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "unit: mark test as a unit test (uses in-memory database, ≤10ms target)"
    )
    config.addinivalue_line(
        "markers", "integration: mark test as an integration test (uses shared database, ≤100ms target)"
    )
    config.addinivalue_line(
        "markers", "e2e: mark test as an end-to-end test (uses file database, ≤1s target)"
    )
    config.addinivalue_line(
