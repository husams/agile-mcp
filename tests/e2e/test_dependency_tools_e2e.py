...
from .test_helpers import (
    validate_full_tool_response, validate_dependency_tool_response,
...
class TestDependencyToolsE2E:
    ...
    def test_backlog_add_dependency_e2e_success(self, mcp_server_process):
        ...
        response = send_jsonrpc_request(mcp_server_process, "tools/call", {
    ...
    def test_backlog_add_dependency_e2e_circular_prevention(self, mcp_server_process):
        ...
        send_jsonrpc_request(mcp_server_process, "tools/call", {
            "name": "backlog.addDependency",
            "arguments": {
                "story_id": story_1_id,
                "depends_on_story_id": story_2_id
    ...
    def test_backlog_add_dependency_e2e_duplicate_dependency(self, mcp_server_process):
        ...
        response1 = send_jsonrpc_request(mcp_server_process, "tools/call", {
            "name": "backlog.addDependency",
            "arguments": {
                "story_id": story_1_id,
                "depends_on_story_id": story_2_id
        ...
        response2 = send_jsonrpc_request(mcp_server_process, "tools/call", {
            "name": "backlog.addDependency",
            "arguments": {
                "story_id": story_1_id,
                "depends_on_story_id": story_2_id
    ...
    def test_backlog_add_dependency_e2e_self_dependency_prevention(self, mcp_server_process):
        ...
        epic_id = create_test_epic(mcp_server_process)
        story_id = create_test_story(mcp_server_process, epic_id, "Self Dependent Story")
        ...
        error_text = result["content"][0]["text"].lower()
        assert ("cannot depend on itself" in error_text or 
                "validation error" in error_text)
    ...
    def test_backlog_add_dependency_e2e_validation_errors(self, mcp_server_process):
        ...
        response1 = send_jsonrpc_request(mcp_server_process, "tools/call", {
            ...
            "arguments": {
                "story_id": "",
                "depends_on_story_id": "some-story-id"
            }
        })
    ...
    def test_backlog_add_dependency_e2e_complex_dependency_graph(self, mcp_server_process):
        ...
        for story_id, depends_on_id in dependencies:
            ...
            assert "error" not in response, f"Failed to add dependency {story_id} -> {depends_on_id}"
    ...
    def test_backlog_add_dependency_e2e_integration_with_existing_tools(self, mcp_server_process):
        ...
        story_1_response = send_jsonrpc_request(mcp_server_process, "tools/call", {
            "name": "backlog.getStory",
            "arguments": {"story_id": story_1_id}
        })
        assert "error" not in story_1_response
        result = story_1_response.get("result", {})
        assert result.get("isError") is not True
        
        story_2_response = send_jsonrpc_request(mcp_server_process, "tools/call", {
            "name": "backlog.getStory", 
            "arguments": {"story_id": story_2_id}
        })
