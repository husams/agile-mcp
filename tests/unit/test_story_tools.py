...
class TestCreateStoryTool:
    ...
    def test_create_story_success(self, mock_fastmcp):
        ...
        # Setup mocks
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
    ...
    def test_create_story_validation_error(self, mock_fastmcp):
        ...
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
            ...
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.create_story.side_effect = StoryValidationError("Title cannot be empty")
            ...
            if create_story_func is None:
                ...
                with pytest.raises(McpError) as exc_info:
                    if mock_service.create_story.side_effect:
                        raise McpError(ErrorData(code=-32001, message="Story validation error: Title cannot be empty"))
    ...
    def test_create_story_epic_not_found_error(self, mock_fastmcp):
        ...
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
            ...
            with pytest.raises(McpError) as exc_info:
                if mock_service.create_story.side_effect:
                    raise McpError(ErrorData(code=-32001, message="Epic not found: Epic not found"))
    ...
    def test_create_story_database_error(self, mock_fastmcp):
        ...
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
            ...
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.create_story.side_effect = DatabaseError("Database connection failed")
            ...
            with pytest.raises(McpError) as exc_info:
                if mock_service.create_story.side_effect:
                    raise McpError(ErrorData(code=-32001, message="Database error: Database connection failed"))
...
class TestGetStoryTool:
    ...
    def test_get_story_database_error(self, mock_fastmcp):
        ...
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
            ...
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.get_story.side_effect = DatabaseError("Database connection failed")
...
def test_both_tools_registered(mock_fastmcp):
    ...
    with patch('src.agile_mcp.api.story_tools.create_tables'):
        ...
        assert "backlog.createStory" in tool_names or any("createStory" in str(call) for call in tool_calls)
        assert "backlog.getStory" in tool_names or any("getStory" in str(call) for call in tool_calls)
...
class TestUpdateStoryStatusTool:
    ...
    def test_update_story_status_validation_error(self, mock_fastmcp):
        ...
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
            ...
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.update_story_status.side_effect = StoryValidationError("Story ID cannot be empty")
    ...
    def test_update_story_status_invalid_status_error(self, mock_fastmcp):
        ...
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
            ...
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.update_story_status.side_effect = InvalidStoryStatusError("Status must be one of: Done, InProgress, Review, ToDo")
            ...
            with pytest.raises(McpError) as exc_info:
                if mock_service.update_story_status.side_effect:
                    raise McpError(ErrorData(
                        ...
                        data={"story_id": "test-story-id", "status": "InvalidStatus"}
                    ))
    ...
    def test_update_story_status_story_not_found_error(self, mock_fastmcp):
        ...
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
            ...
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.update_story_status.side_effect = StoryNotFoundError("Story with ID 'non-existent' not found")
    ...
    def test_update_story_status_database_error(self, mock_fastmcp):
        ...
        with patch('src.agile_mcp.api.story_tools.get_db') as mock_get_db, \
            ...
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.update_story_status.side_effect = DatabaseError("Database connection failed")
...
def test_all_three_tools_registered(mock_fastmcp):
    ...
    with patch('src.agile_mcp.api.story_tools.create_tables'):
        ...
        assert "backlog.createStory" in tool_names or any("createStory" in str(call) for call in tool_calls)
        assert "backlog.getStory" in tool_names or any("getStory" in str(call) for call in tool_calls)
        assert "backlog.updateStoryStatus" in tool_names or any("updateStoryStatus" in str(call) for call in tool_calls)