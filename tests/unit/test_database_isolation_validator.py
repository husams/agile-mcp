...
def test_validate_clean_database_state():
    ...
    try:
        ...
        epic = Epic(id="contamination-test", title="Contamination", description="Test", status="Ready")
        session.add(epic)
        session.commit()
...
def test_validator_contamination_details():
    ...
    engine, session_factory = manager.create_memory_database("contamination_details_test")
    session = session_factory()
