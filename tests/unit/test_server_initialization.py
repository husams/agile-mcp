...
from agile_mcp.main import create_server, main
...
class TestServerInitialization:
    ...
    def test_create_server_returns_fastmcp_instance(self):
        ...
        with patch('agile_mcp.main.logger') as mock_logger:
            ...
            mock_logger.info.assert_called_with("FastMCP server instance created successfully")
    ...
    @pytest.mark.asyncio
    async def test_main_handles_server_creation(self):
        """Test that main function creates and runs server successfully."""
        with patch('agile_mcp.main.create_server') as mock_create_server:
    ...
    @pytest.mark.asyncio
    async def test_main_handles_initialization_errors(self):
        ...
        with patch('agile_mcp.main.create_server') as mock_create_server:
            ...
            with patch('agile_mcp.main.logger') as mock_logger:
                ...
                mock_logger.error.assert_called_with("Server error: Server creation failed")
    
    @pytest.mark.asyncio 
    async def test_main_handles_server_run_errors(self):
        """Test that main function handles server run errors gracefully."""
        with patch('agile_mcp.main.create_server') as mock_create_server:
            mock_server = AsyncMock()
            mock_server.run = AsyncMock(side_effect=Exception("Server run failed"))
            mock_create_server.return_value = mock_server
    ...
    async def test_main_handles_keyboard_interrupt(self):
        ...
        with patch('agile_mcp.main.create_server') as mock_create_server:
            ...
            with patch('agile_mcp.main.logger') as mock_logger:
                await main()  # Should not raise SystemExit for KeyboardInterrupt
                
                # Verify graceful shutdown was logged
                mock_logger.info.assert_any_call("Server shutdown requested via keyboard interrupt")
    ...
    def test_create_server_handles_fastmcp_failure(self):
        ...
        with patch('agile_mcp.main.FastMCP') as mock_fastmcp:
            ...
            with patch('agile_mcp.main.logger') as mock_logger:
                ...
                mock_logger.error.assert_called_with("Failed to create FastMCP server: FastMCP creation failed")
