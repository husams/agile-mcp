...
from tests.e2e.test_helpers import (
    validate_json_response, validate_tool_response_format, 
    validate_jsonrpc_request_format, validate_jsonrpc_response_format,
    validate_mcp_protocol_compliance, validate_story_response,
    validate_epic_response, validate_artifact_response,
    validate_full_tool_response, validate_error_response_format,
    extract_response_data
)
from src.agile_mcp.models.response import StoryResponse, EpicResponse, ArtifactResponse
...
class TestErrorResponseValidation:
    ...
    def test_validate_error_response_format_missing_fields(self):
        ...
        with pytest.raises(pytest.fail.Exception) as exc_info:
            validate_error_response_format(response)
        assert "Error response missing required fields: ['message']" in str(exc_info.value)
