...
def test_link_artifact_to_story_success(artifact_service, mock_repository):
    ...
    mock_repository.create_artifact.assert_called_once_with(
        "file:///path/to/code.js", "implementation", "test-story-id"
    )
...
def test_link_artifact_to_story_empty_story_id(artifact_service, mock_repository):
    ...
    with pytest.raises(ArtifactValidationError, match="Story ID cannot be empty"):
        artifact_service.link_artifact_to_story(
            story_id="", uri="file:///path/to/code.js", relation="implementation"
        )
...
def test_link_artifact_to_story_empty_uri(artifact_service, mock_repository):
    ...
    with pytest.raises(ArtifactValidationError, match="Artifact URI cannot be empty"):
        artifact_service.link_artifact_to_story(
            story_id="test-story-id", uri="", relation="implementation"
        )
    mock_repository.create_artifact.assert_not_called()
...
def test_link_artifact_to_story_empty_relation(artifact_service, mock_repository):
    ...
    with pytest.raises(
        InvalidRelationTypeError, match="Artifact relation cannot be empty"
    ):
        artifact_service.link_artifact_to_story(
            story_id="test-story-id", uri="file:///path/to/code.js", relation=""
        )
    mock_repository.create_artifact.assert_not_called()
...
def test_link_artifact_to_story_uri_too_long(artifact_service, mock_repository):
    ...
    with pytest.raises(
        ArtifactValidationError, match="Artifact URI cannot exceed 500 characters"
    ):
        artifact_service.link_artifact_to_story(
            story_id="test-story-id", uri=long_uri, relation="implementation"
        )
    mock_repository.create_artifact.assert_not_called()
...
def test_link_artifact_to_story_invalid_uri_format(artifact_service, mock_repository):
    ...
    with pytest.raises(
        ArtifactValidationError, match="Artifact URI must be a valid URI format"
    ):
        artifact_service.link_artifact_to_story(
            story_id="test-story-id", uri="not-a-valid-uri", relation="implementation"
        )
    mock_repository.create_artifact.assert_not_called()


def test_link_artifact_to_story_invalid_relation_type(
    artifact_service, mock_repository
):
    """Test artifact linking with invalid relation type."""
    with pytest.raises(
        InvalidRelationTypeError,
        match="Artifact relation must be one of: design, implementation, test",
    ):
        artifact_service.link_artifact_to_story(
            story_id="test-story-id",
            uri="file:///path/to/code.js",
...
def test_link_artifact_to_story_valid_relation_types(artifact_service, mock_repository):
    ...
    for relation in valid_relations:
        ...
        result = artifact_service.link_artifact_to_story(
            story_id="test-story-id", uri="file:///path/to/code.js", relation=relation
        )
...
def test_link_artifact_to_story_model_validation_error(
    artifact_service, mock_repository
):
    """Test artifact linking with model validation error."""
    mock_repository.create_artifact.side_effect = ValueError(
        "Invalid URI format from model"
    )
...
def test_link_artifact_to_story_database_error(artifact_service, mock_repository):
    ...
    mock_repository.create_artifact.side_effect = SQLAlchemyError(
...
def test_get_artifacts_for_story_database_error(artifact_service, mock_repository):
    ...
    mock_repository.find_artifacts_by_story_id.side_effect = SQLAlchemyError(
        "Database connection failed"
    )

    with pytest.raises(
        DatabaseError, match="Database operation failed while retrieving artifacts"
    ):
...
def test_get_artifact_not_found(artifact_service, mock_repository):
    ...
    with pytest.raises(
        ArtifactNotFoundError, match="Artifact with ID 'non-existent' not found"
    ):
...
def test_get_artifact_database_error(artifact_service, mock_repository):
    ...
    mock_repository.find_artifact_by_id.side_effect = SQLAlchemyError(
        "Database connection failed"
    )

    with pytest.raises(
        DatabaseError, match="Database operation failed while retrieving artifact"
    ):
...
def test_link_artifact_to_story_valid_uri_formats(artifact_service, mock_repository):
    ...
    for uri in valid_uris:
        ...
        result = artifact_service.link_artifact_to_story(
            story_id="test-story-id", uri=uri, relation="implementation"
        )
