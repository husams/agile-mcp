...
def test_create_complete_hierarchy(test_session):
    ...
    hierarchy = factory.create_complete_hierarchy(
        epic_count=2, stories_per_epic=2, artifacts_per_story=1
    )
...
def test_factory_cleanup(test_session):
    ...
    # Verify data exists
    assert (
        test_session.query(Epic).filter_by(id="cleanup-test-epic").first() is not None
    )
    assert (
        test_session.query(Story).filter_by(id="cleanup-test-story").first() is not None
    )
...
def test_factory_with_kwargs(test_session):
    ...
    epic = factory.create_epic(epic_id="kwargs-epic", title="Custom Title")
