...
from src.agile_mcp.services.exceptions import EpicValidationError, EpicNotFoundError, InvalidEpicStatusError, DatabaseError
...
def test_update_epic_status_invalid_status(epic_service):
    ...
    invalid_statuses = ["InvalidStatus", "DRAFT", "draft", "Complete", "Finished", "123"]
...
def test_update_epic_status_not_found(epic_service, mock_repository):
    ...
    with pytest.raises(EpicNotFoundError, match="Epic with ID 'nonexistent-id' not found"):
...
def test_update_epic_status_database_error(epic_service, mock_repository):
    ...
    mock_repository.update_epic_status.side_effect = SQLAlchemyError("Database connection failed")
