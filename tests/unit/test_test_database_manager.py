...
def test_context_manager():
    ...
    with manager.get_test_session("unit", "test_003") as session:
        epic = Epic(id="ctx_test", title="Context Test", description="Test", status="Ready")
        session.add(epic)
        session.commit()
...
def test_performance_targets():
    ...
    assert creation_time < 50, f"Database creation took {creation_time}ms, should be ≤50ms"
    ...
    def create_and_test_db(thread_id):
        try:
            with manager.get_test_session("unit", f"thread_{thread_id}") as session:
                ...
                found = session.query(Epic).filter_by(id=f"thread_epic_{thread_id}").first()
...
def test_performance_targets_validation():
    ...
    assert unit_meets_target or integration_meets_target, "At least unit or integration tests should meet performance targets"
